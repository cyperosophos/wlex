...
# TODO: There must be a separate class for unsourced projections, etc.

class Polymor(metaclass=ABCMeta):
    """Base class of polymorphisms

    A polymorphism is a morphism with no source.
    """
    __slots__ = ()

    @abstractmethod
    def to_mor(self, source: Obj) -> 'Mor':
        """Create morphism from polymorphism `self` by setting source"""

    def __repr__(self):
        return f'`unsourced {self!s}`'

class UnsourcedComp(Polymor):
    __slots__ = 'f', 'g', '_comp'
...
# TODO: Obtain composition with transformations through currying.
# This means that there is no special repr of transformations.
# Should repr in other cases just be the name?

class UnsourcedId(Polymor):
    __slots__ = ()
...
class Category:
    ...
    def eq(
        ...
        if isinstance(ssource, Polymor):
            if isinstance(starget, Polymor):
                raise Error
            #if not isinstance(starget, Mor):
            #    raise TypeError
            ssource = ssource.to_mor(starget.source)
        elif isinstance(starget, Polymor):
            #if not isinstance(ssource, Mor):
            #    raise TypeError
            starget = starget.to_mor(ssource.source)
...
class CheckedCategory:
    ...
    def eq(
            self, name,
            ssource: Mor | Polymor | Obj,
            starget: Mor | Polymor | Obj,
            proof: Eq | Mor | Obj | None = None,
        ):
