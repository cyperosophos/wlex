...
class LabeledObj(CartObj):
    __slots__ = 'label', 'obj'
    label: str
    obj: Obj

    # Relabel. The labels of a product component get automatically used by the product.
    # Conversion: relabeling creates a non identical type.
    # Simplification: LabeledObj allows all component types of a product to be different.
    # If two types are identical (same label), then pullback follows.

    def __init__(self, label: str, obj: Obj):
        if not label:
            raise ValueError("Non-empty `label` required")
    ...
    def relabel(self, label: str):
        if label:
            return LabeledObj(label, self.obj)

        return self

    def delabeling(self):
        return Delabeling(self)

    # `conversion` is only supported from the delabeled object.

    def hint(self):
        return self.label, self.obj

    def identical(self, x: Obj):
        return super().identical(x) or (
            isinstance(x, LabeledObj)
            and self.label == x.label
            and self.obj.identical(x.obj)
        )

    def accepts(self, x: object):
        return self.obj.accepts(x)

    def same(self, x: object, y: object):
        return self.obj.same(x, y)

class Delabeling(CartMor):
    __slots__ = ()

    def __init__(self, labeled: LabeledObj):
        # TODO: This includes deep delabeling: composition of delabelings is just a delabeling.
        super().__init__(labeled, labeled.obj)
...
class LabeledMor(CartMor):
    __slots__ = ('mor',)
    mor: Mor

    def __init__(self, label: str, mor: Mor):
        self.mor = mor
        super().__init__(mor.source, LabeledObj(label, mor.target))

    # TODO: Composing with delabeling always gives `mor`.

    def same(self, x: Mor):
        return super().same(x) or (
            isinstance(x, LabeledMor)
            and self.target.identical(x.target)
            and self.mor.same(x.mor)
        )

    def hint(self):
        return self.target, self.mor

    def ev(self, x: object):
        return self.mor.ev(x)
...
class Product(CartObj):
    ...
    __slots__ = 'components', 'obj_to_idx', 'label_to_obj', 'overlaps'

    components: list[Obj]
    obj_to_idx: dict[Obj, tuple[int, ...]] # Maps to the first index with label
    label_to_obj: dict[str, LabeledObj]
    overlaps: dict[tuple[int, ...], tuple[int, ...]]

    def flat_length(self) -> int:
        """Length of product including the length of subproducts"""
        return len(self.components) + sum(
            p.flat_length() - 1 for p in self.components
    ...
    def label_components(
        ...
        labels: Sequence[str],
        offset: int = 0,
        replacements: dict[LabeledObj, LabeledObj] | None = None,
    ) -> 'Product':
        # The new label for a repeated label must be empty or repeated.
        if replacements is None:
            replacements = {}

        new_components: list[Obj] = []
        for component in self.components:
            if isinstance(component, Product):
                new_components.append(component.label_components(
                    labels, offset=offset, replacements=replacements,
                ))
                offset += component.flat_length()
                continue

            label = labels[offset]
            if isinstance(component, LabeledObj):
                nc = component.relabel(label)
            elif label:
                nc = LabeledObj(label, component)
            elif component in replacements:
                # No label. Use previous replacement.
                nc = replacements[component]
            else:
                nc = component

            if component in replacements:
                prev = replacements[component]
                if not prev.identical(nc):
                    raise ValueError("Overlap must be kept")
            else:
                replacements[component] = nc

            new_components.append(nc)
            offset += 1

        return Product(new_components)

    def __new__(cls, components: Sequence[Obj], no_repeat: bool = False):
        if not components:
    ...
    def _update_mappings_from_product(self, component: 'Product', idx: int, no_repeat: bool):
        for label in component.label_to_obj:
            if label in self.label_to_obj:
                if no_repeat:
                    raise ValueError("Can't repeat component in product component")

                if not self.label_to_obj[label].identical(component):
                    raise ValueError(
                        "Repeated label requires repeated associated object.",
                    )

    def _update_mappings(self, component: Obj, idx: int):
        if isinstance(component, LabeledObj):
            ...
            if label in self.label_to_obj:
                ...
                self.label_to_obj[label] = component



    def __init__(self, components: Sequence[Obj], no_repeat: bool = False):
        ...
        # ONE MUST! allow repeated unlabeled types. This is required in order to have something like lists.
        # This can be a different type. So that on the one hand one has (simple) Product which is based
        # on component order for conversions, and on the other one has ParameterProduct which must have each
        # component of a different type. Map and Product.
        # Alternative: simplify implicit conversion (allow other explicit conversions).
        # Two modes: list mode, dict mode. pullbacks only occur under dict mode.
        # All components must be provided with labels, except for products, which always
        # get flattened. Product has a relabeling method. The List monad is not a builtin but an emergent
        # construct that arises from assuming that a certain morphism is exponentiable.
        # For efficiency a builtin construct can be provided which is almost equivalent.
        # From the perspective of extensive categories, products are functions from finite sets
        # not just subsets of N.
        # When merging subobjects there is no need to precompose with a projection in the case of flattened
        # product, all other requirements get simply precomposed with the projection of the label.
        ...
        if len(components) == 1:
            raise ValueError("Single component not allowed")
        ...
        for idx, component in enumerate(components):
            if isinstance(component, Product):
                self._update_mappings_from_product(component, idx, no_repeat)
    ...
    def __init__2(self, components: Sequence[LabeledObj], no_repeat: bool = False):
        # Repeated component labels are allowed. This is based on the fact that
        # AxBxC is the pullback of the projections AxB->B and BxC->B.
